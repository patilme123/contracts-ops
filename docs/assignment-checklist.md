...
| --- | --- | --- |
| Organisation workspace | Complete | Demo access selects the current organisation; its profile and seeded team are available in the Organisation view. |
...
| Backend search and filters | Complete | One universal search field plus status, PO date, sort, and page-size controls call the REST API. |
...
| Status, date, and universal search | Complete | Prisma filters support status, PO date ranges, partial case-insensitive client name, UUID, and contract number. |
...
| Seed data | Complete | Idempotent seed provides 2 organisations, 6 team members, and 5 contracts across all statuses. |
...
| API and business workflow tests | Complete - 21 automated tests |
